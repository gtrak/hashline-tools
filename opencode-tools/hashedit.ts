...
export default tool({
  description: "Edit file using hash-anchored line references",
  args: {
        ...
        tool.schema.union([
          ...
          tool.schema.object({
            insert_after: tool.schema.object({
              anchor: tool.schema
                .string()
                .describe('Insert after this "LINE:HASH"'),
  ...
  async execute(args, context) {
    ...
    const result =
      await $`hashline-tools edit ${filepath} --edits ${editsJson}`;
