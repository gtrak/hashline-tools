...
/// Normalize line for comparison (remove trailing \r, collapse internal whitespace)
...
fn to_base36(mut n: u64) -> String {
    ...
    for _ in 0..4 {
        ...
        chars.push(if rem < 10 { b'0' + rem } else { b'a' + rem - 10 });
        n /= RADIX;
    }
    chars.reverse();
...
fn find_fuzzy_match(content: &str, old_text: &str) -> Result<(usize, String), String> {
    ...
    // Single best match
    let (line_num, sim, matched_line) = best_matches[0];
...
#[derive(Debug, Deserialize)]
pub struct SetLineEdit { anchor: String, new_text: String }
#[derive(Debug, Deserialize)]
pub struct ReplaceLinesEdit { start_anchor: String, end_anchor: String, new_text: String }
#[derive(Debug, Deserialize)]
pub struct InsertAfterEdit { anchor: String, text: String }
#[derive(Debug, Deserialize)]
pub struct ReplaceEdit { old_text: String, new_text: String, all: Option<bool> }
...
fn cmd_read(file_path: &str, offset: Option<usize>, limit: Option<usize>) -> Result<String, String> {
...
mod tests {
    ...
    fn test_cmd_edit_replace_lines() {
        ...
        let edits = format!(r#"[{{"type":"replace_lines","start_anchor":"2:{}","end_anchor":"2:{}","new_text":"replaced line"}}]"#, hash2, hash2);
